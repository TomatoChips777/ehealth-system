...
function AnnualPhysicalExamForm() {
  ...
  const [formData, setFormData] = useState({
    patient_id: patient?.user_id || basic_info?.user_id || '',
  ...
  const fetchExamData = async () => {
    ...
    try {
      const response = await axios.get(`${import.meta.env.VITE_GET_ANNUAL_PHYSICAL_EXAM_BY_ID_2}/${patient?.user_id}`);
      const examData = response.data.exam;
  
      // Fetch student logs
      const logsResponse = await axios.get(`${import.meta.env.VITE_GET_STUDENT_LOGS_BY_ID}/${patient?.user_id}`);
      ...
      console.error('Error fetching exam or logs:', error);
      setLoading(false);
    }
  };
