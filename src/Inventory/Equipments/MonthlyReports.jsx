...
function EquipmentMonthlyReport() {
  const [reportData, setReportData] = useState([]);
  const [loading, setLoading] = useState(true);
  const navigate = useNavigate(); 

  const groupByMonth = (equipments) => {
    const itemMap = {};
  ...
  const handleDownloadPDF = () => {
    const columns = ['Equipment Inventory', ...MONTHS, 'Total', 'Remarks'];
    ...
    exportToPDF('Equipment Inventory Report (Aug - Jul)', columns, data);
  };
  ...
  return (
    <Container className="p-0 y-0" fluid>
      <Card className='p-3'>
        <h2 className="text-center mb-4">Equipments Inventory Report (Aug - Jul)</h2>
...
export default EquipmentMonthlyReport;
