...
function AppoinmentCalendar() {
  ...
  const fetchAvailableTimes = async (date) => {
    try {
      ...
      // All available slots (those marked as available)
      const availableSlots = response.data.availability
        .filter(slot => slot.is_available === 1)
        .map(slot => slot.time_slot.slice(0, 5)); // Normalize to "HH:mm"

      // Normalize function
      const normalizeTime = (time) => time.slice(0, 5); // "HH:mm"

      // Booked times from events/appointments
      const bookedTimes = [...eventPlans, ...appointments]
        .filter(app => formatLocalDate(app.date) === date)
        .map(app => normalizeTime(app.time));
  ...
  const handleAvailabilityChange = async (date, time, isAvailable) => {
    try {
  ...
  const toggleTimeSlot = (date, time) => {
    ...
    handleAvailabilityChange(date, time, newAvailability);
  };
  ...
  return (
    <div className="container-fluid p-0 y-0">
              ...
              return (
                <div
                  ...
                  style={{
                    width: '14.2%',
                    minHeight: '70px',
                    backgroundColor: isAvailable || hasAppointment ? '#4CAF50' : '#f8f9fa',
                    color: isAvailable || hasAppointment ? 'white' : '#6c757d',
                    fontSize: '0.75rem',
                    cursor: isAvailable ? 'pointer' : 'not-allowed',
                    overflow: 'hidden',
                    pointerEvents: isAvailable ? 'auto' : 'none'  // This disables clicking
                  }}
                  onClick={() => {
      ...
      <Modal show={showAvailabilityModal} onHide={() => setShowAvailabilityModal(false)}>
        ...
        <Modal.Body>
          <p><strong>Date:</strong> {FormatDate(clickedDate, false)}</p>
